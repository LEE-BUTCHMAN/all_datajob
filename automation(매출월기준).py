...
def update_delivery_data_by_row(df, delivery_type, item_rows, worksheet):
    ...
    print(f"{delivery_type} {target_month}월을 {chr(64 + target_col)}열에 업데이트합니다.")

    # 해당 월 데이터만 찾기
    target_month_data = df[df['entering_month'] == target_month]

    if target_month_data.empty:
        print(f"{delivery_type} {target_month}월 데이터가 없습니다.")
        return

    for _, row in target_month_data.iterrows():
        print(f"{delivery_type} {target_month}월 업데이트 중... (열: {target_col})")
        ...
        print(f"{delivery_type} {target_month}월 완료!")
        break  # 해당 월은 하나만 있으므로 break


def update_sheets(direct_df, parcel_df):
    """Google Sheets에 직배/택배 데이터 업데이트 - 설정된 월을 해당 열에 업데이트"""
    # 인증
...
def update_bs_segment_sheets(direct_df, parcel_df):
    """Google Sheets에 금액 구간별 비중 데이터 업데이트 - 설정된 월을 해당 열에 업데이트"""
    # 인증
    scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive']
    creds = Credentials.from_service_account_file('/Users/sfn/Downloads/automation-data-467003-6310e37f0e5c.json',
                                                  scopes=scope)
    ...
    def update_bs_data_by_row(df, delivery_type, bs_rows):
        """배송 유형별 금액 구간 비중 데이터 업데이트 - 설정된 월을 해당 열에 입력"""
        ...
        if target_month_data.empty:
            print(f"{delivery_type} {target_month}월 금액구간 데이터가 없습니다.")
            return

        for _, row in target_month_data.iterrows():
            print(f"{delivery_type} {target_month}월 금액구간 비중 업데이트 중... (열: {target_col})")
            ...
            print(f"{delivery_type} {target_month}월 금액구간 비중 완료!")
            break  # 해당 월은 하나만 있으므로 break
...
def update_total_bs_segment_sheets(total_bs_df):
    ...
    print(f"전체 {target_month}월을 {chr(64 + target_col)}열에 업데이트합니다.")

    # 해당 월 데이터만 찾기
    target_month_data = total_bs_df[total_bs_df['entering_month'] == target_month]

    if target_month_data.empty:
        print(f"전체 {target_month}월 금액구간 데이터가 없습니다.")
        return

    # 해당 월 데이터 사용 (직배+택배 합계 결과)
    for _, row in target_month_data.iterrows():
        print(f"전체 {target_month}월 금액구간 비중 업데이트 중... (열: {target_col})")
        ...
        print(f"전체 {target_month}월 금액구간 비중 완료!")
        break  # 해당 월 데이터 처리 완료


def update_sheets_business(business_data):
    """Google Sheets에 업종별 데이터 업데이트 - 설정된 월을 해당 열에 업데이트"""
    # 인증
    scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive']
    creds = Credentials.from_service_account_file('/Users/sfn/Downloads/automation-data-467003-6310e37f0e5c.json',
                                                  scopes=scope)
    ...
    def update_business_data_by_row(df, business_name, start_row):
        ...
        print(f"{business_name} {target_month}월을 {chr(64 + target_col)}열에 업데이트합니다.")
        ...
        if target_month_data.empty:
            print(f"{business_name} {target_month}월 데이터가 없습니다.")
            return

        for _, row in target_month_data.iterrows():
            print(f"{business_name} {target_month}월 업데이트 중... (열: {target_col})")
            ...
            print(f"{business_name} {target_month}월 완료!")
            break  # 해당 월은 하나만 있으므로 break
...
def main():
    """모든 데이터 업데이트 - 설정된 월을 해당 열에 업데이트"""
    print(f"{TARGET_MONTH}월 직배/택배 + 업종별 데이터 {chr(64 + 2 + (TARGET_MONTH - 7))}열 업데이트 시작...")
    ...
    print(f"{TARGET_MONTH}월 {chr(64 + 2 + (TARGET_MONTH - 7))}열 업데이트 완료!")
